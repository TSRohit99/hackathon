    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                const response = await axios.post(`${process.env.NEXT_PUBLIC_DOMAIN}/api/user/forgot-password`, { ...values });
                dispatch(toggleButtonLoading(false));
                showToastWithCloseButton(response.data.message, 'success');
            ...
            <form onSubmit={formik.handleSubmit} className='flex flex-col items-center gap-3 p-1'>
                ...
                <div className='w-full'>
                    ...
                    <div className='flex justify-between rounded border border-slate-300 outline-none px-2 py-1 text-slate-500 font-medium'>
                        ...
                        />
                        <button onClick={(e) => { e.stopPropagation(); setIsPasswordShow(prev => !prev); }}>
                ...
                <button type='submit' className='w-full bg-tomato_1 px-2 py-1 text-white font-semibold rounded'>
                    {isButtonLoading ? <span className='loading loading-spinner loading-xs text-slate-500'></span> : "Send Confirmation"}
                </button>
